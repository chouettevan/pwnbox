...
int main(int argc, char *argv[]) {
  ...
  seccomp_rule_add(context, SCMP_ACT_KILL_PROCESS, SCMP_SYS(ptrace), 0);
  seccomp_rule_add(context, SCMP_ACT_ALLOW, SCMP_SYS(write), 1,
                   SCMP_A0(SCMP_CMP_EQ, 1));
  seccomp_rule_add(context, SCMP_ACT_ALLOW, SCMP_SYS(mmap), 0);
  seccomp_rule_add(context, SCMP_ACT_ALLOW, SCMP_SYS(brk), 0);
  seccomp_rule_add(context, SCMP_ACT_LOG, SCMP_SYS(mmap), 1,
                   SCMP_A2(SCMP_CMP_MASKED_EQ, PROT_EXEC, 1));
